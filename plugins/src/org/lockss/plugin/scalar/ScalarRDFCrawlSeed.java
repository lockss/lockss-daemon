...
public class ScalarRDFCrawlSeed extends BaseCrawlSeed {

    private static final Logger log = Logger.getLogger(ScalarRDFCrawlSeed.class);
    ...
    public ScalarRDFCrawlSeed(Crawler.CrawlerFacade facade) {
        super(facade);
        if (au == null) {
    ...
    protected void initialize() throws ConfigurationException, PluginException, IOException {
        ...
        this.collectionType = au.getConfiguration().get(COLLECTION_TYPE_KEY);
    ...
    protected void populateUrlList() throws IOException {
        ...
        String apiStartUrl = this.baseUrl + this.journalID + "/rdf/instancesof/" + this.collectionType;

        log.debug2("apiStartUrl: " + apiStartUrl);

        ScalarRDFLinkExtractor ple = new ScalarRDFLinkExtractor();
    ...
    protected UrlFetcher makeApiUrlFetcher( final ScalarRDFLinkExtractor ple,
                                            final String url,
