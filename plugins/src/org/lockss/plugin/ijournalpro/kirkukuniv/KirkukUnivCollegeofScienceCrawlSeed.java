...
public class KirkukUnivCollegeofScienceCrawlSeed extends BaseCrawlSeed {

    private static final Logger log = Logger.getLogger(org.lockss.plugin.ijournalpro.kirkukuniv.KirkukUnivCollegeofScienceCrawlSeed.class);

    //https://kujss.uokirkuk.edu.iq/browse?_action=issue
    ...
    protected String browseIssueUrl;
    protected String volume;


    public KirkukUnivCollegeofScienceCrawlSeed(Crawler.CrawlerFacade facade) {
        super(facade);
        if (au == null) {
    ...
    protected void initialize() throws ConfigurationException, PluginException, IOException {
        ...
        this.volume = au.getConfiguration().get("volume");
        this.browseIssueUrl = this.baseUrl + "browse?_action=issue";
        this.urlList = null;
    }
    ...
    protected void populateUrlList() throws IOException {
        ...
        String storeUrl = String.format("%slockss-generated/%s",this.baseUrl,this.volume);

        String apiStartUrl = this.browseIssueUrl;

        log.debug2("apiStartUrl: " + apiStartUrl);

        LinkExtractor defaultExtractor = null;

        LinkExtractor ple = new KirkukUnivCollegeofScienceVolumeIssueLinkExtractor(defaultExtractor);

        UrlFetcher uf = makeApiUrlFetcher((KirkukUnivCollegeofScienceVolumeIssueLinkExtractor) ple, apiStartUrl, this.browseIssueUrl);
        ...
        try {
            ...
            if(ce.getCause() != null && ce.getCause().getMessage().contains("LOCKSS")) {
                log.debug("Result errored due to LOCKSS audit proxy. Trying alternate start Url", ce);
     ...
     * using the given KirkukIssueLinkExtractor instance.
     * </p>
     *
     * @param ple
     *          A  KirkukIssueLinkExtractor instance to parse the API
     *          response with.
     * @param url
     *          A query URL.
     ...
     * @since 1.67.5
     */
    protected UrlFetcher makeApiUrlFetcher( final KirkukUnivCollegeofScienceVolumeIssueLinkExtractor ple,
                                            final String url,
                                            final String browseIssueUrl) {
        // Make a URL fetcher
                            ...
                            ple.extractUrls(au,
                                    ...
                                    new LinkExtractor.Callback() {
                                        ...
                                        public void foundLink(String issueUrl) {
                                            log.debug3("issueUrl is added = " + issueUrl);
