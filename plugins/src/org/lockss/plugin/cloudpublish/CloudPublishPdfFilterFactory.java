...
public class CloudPublishPdfFilterFactory extends ExtractingPdfFilterFactory {

  private static final Logger log = Logger.getLogger(CloudPublishPdfFilterFactory.class);
  /*
   This document was generated by CloudPublish for CLOCKSS at 171.66.236.68 on 2022-02-02, 07:07:34 1643785654GMTC
   */

  protected static final Pattern WATERMARK_LINE_1 =
      Pattern.compile("^This document was generated by CloudPublish for C?LOCKSS at");
      ...
      Iterator<PdfToken> filteredIter = new SubsequenceIterator<PdfToken>(tokenIter) {
        ...
        protected void hasAccumulated() {
          for (PdfToken tok : accumulator) {
            log.info("Doing a tok: " + tok.toString());
            boolean foundIt = false;
            if (tok.isArray()) {
              StringBuilder tokArr = new StringBuilder("");
              for (PdfToken tok2 : tok.getArray()) {
                log.info("--    doing a tok2: " + tok2.toString());
                if (tok2.isString()) {
                  if (tok2.getString().startsWith("þ")) {
                    foundIt = true;

                  }
                  tokArr.append(tok2.getString());
                }
                if (tok2.isFloat()) {
                  log.info("---------------- float: " + tok2.getFloat());

                }
              }
              log.info(" -------------- stringBuilt: " + tokArr);
            }
            if (tok.isOperator()) {
              String op = tok.getOperator();
  ...
  public static void main(String[] args) throws Exception {
    String[] fileStrs = {
        "/home/mark/Downloads/c1_av55_57735.pdf",
        "/home/mark/Downloads/c3_av55_57735.pdf",
    };
    for (String fileStr : fileStrs) {
      FilterFactory fact = new CloudPublishPdfFilterFactory();
