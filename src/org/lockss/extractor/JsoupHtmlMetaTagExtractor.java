...
public class JsoupHtmlMetaTagExtractor extends SimpleFileMetadataExtractor {
  static Logger theLog = Logger.getLogger("JsoupHtmlMetaTagExtractor");

  static final String DEFAULT_META_TAG = "meta";
  protected Collection<String> m_tags;

  /**
   * default constructor which will process meta tags only
   * kept for backwards compatiblity
   */
  public JsoupHtmlMetaTagExtractor() {
  }

  /**
   * Create an extractor what will extract the value(s) of the html tags in
   * <code>tags</code>
   * @param tags the list of html tags whose value to extract
   */
  public JsoupHtmlMetaTagExtractor(Collection<String> tags) {
    m_tags = tags;
  }
   ...
   */
  public JsoupHtmlMetaTagExtractor(Map tagMap) {
  ...
  public ArticleMetadata extract(final MetadataTarget target,
    ...
    if(cu.getContentSize() > 0) {
      ...
      // we pass in null to make sure we don't actually
  ...
  void extractTags(Document doc, ArticleMetadata articleMeta)
  {
    ...
    for(String tag : m_tags) {
      ...
      for(Element tag_el : tag_elements) {
        if(tag_el.hasText()) {
          val = processHtml(tag, tag_el.text());
          articleMeta.putRaw(tag, val);
        }
      }
    }
  }
  ...
  /**
   * take the value for a tag from an html page and perform the necessary
   * transformations to regularize it for storing in the article metadata.
   * this will strip embedded html tags, unescape any escaped html and remove
   * any extra spaces.
   *
