...
public class ScheduledChecksumUpdater {
  ...
  public static boolean updateAU(ArchivalUnit au) throws Exception {
  ...
  private static class VerifierEventHandler implements BlockHasher.EventHandler, HashService.Callback {
    ...
    public void blockDone(HashBlock hblock) {
      ...
      if ( ver.getHashError() != null ) {
        ...
        headers.setProperty(CachedUrl.PROPERTY_CHECKSUM, sdigest);
        UrlCacher uc = au.makeUrlCacher(url);
        InputStream is = cu.getUnfilteredInputStream();
        try {
          uc.storeContent(is, headers);
        } catch (IOException ex) {
          logger.error(String.format("Got exception while trying to update local hash of Url %s", url), ex);
        } finally {
