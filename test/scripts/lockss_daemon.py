...
class Framework:
    ...
    def __init__(self, frameworkDir, username, password,
        ...
        for port in range(self.startPort, self.startPort + self.daemonCount):
            ...
            daemonDir = path.abspath(path.join(self.frameworkDir,
                                               'daemon-' + str(port)))
            # create client
            client = Client(daemonDir, url, username, password)
            # local config
    ...
    def __writeLocalConfig(self, file, dir, port):
        """ Write local config file for a daemon.  Daemon num is assumed to start
        at 0 and go up to n, for n-1 daemons """
        f = open(file, 'w')
        ...
        f.write(localConfigTemplate % (dir, dir, proxyStart, port, ipAddr,
                                       unicastPort, unicastSendToPort))
        f.close()
...
class Client:
    ...
    def getAuNodesWithContent(self, au):
        ...
        tab = self.__getStatusTable('ArchivalUnitTable', au.getAuId())
        nodes = []
        for row in tab:
            url = row['NodeName']
            if not row['NodeContentSize'] == '-':
    ...
    def waitForDamage(self, au, node=None, timeout=DEF_TIMEOUT, sleep=DEF_SLEEP):
        ...
        timeout expires.  If 'node' is None, this will just wait until the
        whole AU is marked 'repairing'"""
        def waitFunc():
    ...
    def waitForNameRepair(self, au, node=None, timeout=DEF_TIMEOUT, sleep=DEF_SLEEP):
        """ Block until a node has been successfully repaired, or until
    ...
    def getAuNode(self, au, url, checkForContent=False):
        ...
        root = path.join(repository, au.dirStruct)
    ...
    def __getStatusTable(self, statusTable, key=None):
...
localConfigTemplate = """\
org.lockss.platform.diskSpacePaths=%s
org.lockss.cache.location=%s
org.lockss.proxy.start=%s
org.lockss.ui.port=%s
org.lockss.localIPAddress=%s
org.lockss.comm.unicast.port=%s
org.lockss.comm.unicast.sendToPort=%s
