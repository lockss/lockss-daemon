...
while (my $line = <>) {
  ...
  if ($plugin eq "HighWirePressH20Plugin" || $plugin eq "HighWirePressPlugin") {
  ...
  } elsif ($plugin eq "OUPDrupalPlugin" || $plugin eq "APSDrupalPlugin" || $plugin eq "BMJDrupalPlugin") {
  ...
  } elsif ($plugin eq "ClockssOUPDrupalPlugin" || $plugin eq "ClockssAPSDrupalPlugin") {
    ...
    if ($resp->is_success) {
      my $man_contents = $resp->content;
      if (defined($man_contents) && (($man_contents =~ m/$lockss_tag/) || ($man_contents =~ m/$clockss_tag/)) && ($man_contents =~ m/Issue.aspx/)) {
        $result = "Manifest"
      } else {
        $result = "--NO_TAG--"
      }
