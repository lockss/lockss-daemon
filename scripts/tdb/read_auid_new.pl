...
while (my $line = <>) {
  chomp $line;
  my $auid_long = $line;
  my @input_strings = split(/\t/, $line); #either 1 element (auid) or 6 (auid,issn,eissn,year,volume,title)
  my $num_values = int(@input_strings); #either 1 or 6
  if ($num_values == 6) {
    $auid_long = $input_strings[0];
#    $au_type = $input_strings[1];
    $au_issn = $input_strings[1];
    $au_eissn = $input_strings[2];
    $au_year = $input_strings[3];
    $au_volume = $input_strings[4];
...
#    printf("*Elements: %s %s %s %s %s\n", $au_type, $au_issn, $au_eissn, $au_year, $au_title); #debug old
#    printf("*Elements: %s %s %s %s %s\n", $au_issn, $au_eissn, $au_year, $au_volume, $au_title); #debug new
  }
...
#CLOCKSS HighWire HW Origins & Scolaris
    ...
    if ($resp->is_success) {
      ...
      if ($resp->is_success) {
          ...
          if ($req->url ne $resp->request->uri) {
              ...
              $vol_title = $param{collection_id} . " " . $param{year};
              $result = "Manifest"
          } else {
              $result = "--NO_TAG--"
          }
  ...
  } elsif ($plugin eq "GovInfoCourtsSitemapsPlugin") {
      #https://www.govinfo.gov/sitemap/USCOURTS/USCOURTS_akd_2014_sitemap.xml
      $url = sprintf("%ssitemap/USCOURTS/USCOURTS_%s_%d_sitemap.xml",
      $param{base_url}, $param{court_id}, $param{year});
      $man_url = uri_unescape($url);
      my $req = HTTP::Request->new(GET, $man_url);
      my $resp = $ua->request($req);
      if ($resp->is_success) {
          ...
          if ($req->url ne $resp->request->uri) {
              ...
              $vol_title = $param{collection_id} . " " . $param{court_id} . " " . $param{year};
              $result = "Manifest"
          } else {
              $result = "--NO_TAG--"
          }
...
#        } elsif ($man_contents =~ m/="\/(Library\/SearchLinkRedirect\?folder=[^"]*)">/si) {
         } elsif ($man_contents =~ m/href="\/(Library\/SearchLinkRedirect\?folder=[^"]*)"/si) {
            my $art_url = $param{base_url} . $1;
            $man_art_url = uri_unescape($art_url);
            my $req_art = HTTP::Request->new(GET, $man_art_url);
            my $resp_art = $ua->request($req_art);
...
#the url can be built up from the available parameters
  ...
  } elsif ($plugin eq "ClockssHBKUPlugin") {
...
#  } elsif (($plugin eq "SilverchairJournalsPlugin") || ($plugin eq "AmaSilverchairPlugin")) {
  } elsif ($plugin eq "AmaSilverchairPlugin") {
...
#  } elsif (($plugin eq "ClockssSilverchairJournalsPlugin") || ($plugin eq "ClockssAmaSilverchairPlugin")) {
  } elsif ($plugin eq "ClockssAmaSilverchairPlugin") {
    $url = sprintf("%sLOCKSS/ListOfIssues.aspx?resourceId=%d&year=%d",
      $param{base_url}, $param{resource_id}, $param{year});
    $man_url = uri_unescape($url);
    my $req = HTTP::Request->new(GET, $man_url);
...
#      } elsif (defined($man_contents) && ($man_contents =~ m/\/$param{journal_id}\[^"\]*-volume-$param{volume_name}\[^"\]*\"/)) {
  ...
  # Plugins for small publishers. It's not worth it to write a complete manifest page checker
          ...
          ($plugin eq "ClockssGhentUniversityLibraryDomainModeJournalsPlugin") ||
          ($plugin eq "ClockssSulaimaniDentalJournalsPlugin") ||
          ($plugin eq "ClockssUniversityOfLeedsBooksPlugin") ||
          ($plugin eq "ClockssUnivofAnbarCollegeofAgJournalsPlugin") ||
          ($plugin eq "GhentUniversityLibraryDomainModeJournalsPlugin") ||
