  ...
  #University of Michigan Press Books
    ...
    if ($resp->is_success && $p_resp->is_success) {
      ...
      if ($req->url ne $resp->request->uri) {
        ...
        if (($man_contents !~ m/Your institution does not have access to this book/) && ($man_contents =~ m/locale=en/) && ($man_contents =~ m/a data-rep-type=\"pdf\"/) && ($man_contents =~ m/Access to this book has been provided by your Library/ || ($man_contents =~ m/Open Access/ && $man_contents =~ m/\(open access\)/ ) )) {
          if ($man_contents =~ m/<title>\s*(\S[^<]*\S)\s*<\/title>/si) {
            $vol_title = $1;
            $vol_title =~ s/\s*\n\s*/ /g;
            if (($vol_title =~ m/</) || ($vol_title =~ m/>/)) {
