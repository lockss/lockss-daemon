...
# Make a list of AUids that are crawling in clockssingest, manifest in gln
   ...
   ./scripts/tdb/tdbout -CZLI -a -Q "year ~ '2013' and plugin ~ '$plugin' and year != '2016'" tdb/clockssingest/*.tdb | sort > $tpath/gr_clockss_c.txt
   # Make a list of AUids from gln
   ./scripts/tdb/tdbout -M -a -Q "year ~ '2013' and plugin ~ '$plugin' and year != '2016'" tdb/prod/*.tdb | sort > $tpath/gr_gln_m.txt
