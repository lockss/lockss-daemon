...
# Does AU have a clockss and gln manifest page?
   ...
   cat $tpath/gr_common_shuf_c.txt | sed -e 's/HighWirePressH20Plugin/ClockssHighWirePressH20Plugin/' > $tpath/gr_common_manifest.txt
   # Find common items on the list of AUs with manifest pages, and the list of healthy AUs on the ingest machines.
   comm -12 $tpath/gr_ingest_healthy.txt $tpath/gr_man_gln.txt
