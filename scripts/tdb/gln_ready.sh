...
# Make a list of AUids that are crawling in clockssingest, manifest in gln
   ...
   ./scripts/tdb/tdbout -CZLI -a -Q "year ~ '^20' and plugin ~ '$plugin' and year !~ '2017'" tdb/clockssingest/tay*.tdb | sort > $tpath/gr_clockss_c.txt
   ...
   ./scripts/tdb/tdbout -M -a -Q "year ~ '^20' and plugin ~ '$plugin' and year !~ '2017'" tdb/prod/tay*.tdb | sort > $tpath/gr_gln_m.txt
