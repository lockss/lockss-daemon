...
print_usage() {
  echo "Usage: no flags for complete list."
...
#echo "Plugin,Name,GIT,C5-8,D1-13,C1-4,I1-4,Parent,PV,AUs,Status"
echo "Plugin,Name,GIT,AUs,Journals,Books,Status"
...
#include all plugins
join -t, -e EMPTY $tpath/blatz18.txt $tpath/blatz08.txt | join -t, -e EMPTY - $tpath/blatz00.txt | join -t, -e EMPTY - $tpath/blatz19.txt | join -t, -e EMPTY - $tpath/blatz20.txt | join -t, -e EMPTY - $tpath/blatz01.txt | sort -t, -k 10 -nr 
#else
#include only plugins that don't have a status that starts with "deprecated" or "ready"
#join -t, -e EMPTY $tpath/blatz18.txt $tpath/blatz08.txt | join -t, -e EMPTY - $tpath/blatz00.txt | join -t, -e EMPTY - $tpath/blatz16.txt | sort -t, -k 10 -nr 
#fi
date
