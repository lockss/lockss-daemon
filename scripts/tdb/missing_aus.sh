...
########GLN
./scripts/tdb/tdbout -MWTNYR -Q 'year ~ "2016" and plugin !~ "Book" and plugin !~ "ClockssGSWDrupalPlugin"' -t publisher,title tdb/prod/*.tdb | sort > $t/xg2016
...
#########CLOCKSS
./scripts/tdb/tdbout -MWTNYCZIF -Q 'year ~ "2016" and plugin !~ "Book" and plugin !~ "ClockssGSWDrupalPlugin"' -t publisher,title tdb/clockssingest/*.tdb | sort > $t/xc2016
