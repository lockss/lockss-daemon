...
echo -e "Publisher\tJournal\tISSN\tEISSN\tVolume\tYear" > ../SageEdits/JournalsReleasedToday.tsv ; scripts/tdb/tdbout -G -Q 'type is not "bookSeries" and type is not "book"' -t publisher,title,issn,eissn,name,year tdb/prod/ | sort >> ../SageEdits/JournalsReleasedToday.tsv
echo -e "Publisher\tJournal\tISBN\tEISBN\tVolume\tYear" > ../SageEdits/BooksReleasedToday.tsv ; scripts/tdb/tdbout -G -Q 'type is "bookSeries" or type is "book"' -t publisher,title,isbn,eisbn,name,year tdb/prod/ | sort >> ../SageEdits/BooksReleasedToday.tsv
