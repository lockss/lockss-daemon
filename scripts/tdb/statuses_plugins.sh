...
shopt -s extglob
cat ../../tdb/!(clockss)/*.tdb | ../../scripts/tdb/tdbout -t plugin | sed 's/\./\//g' | awk -F"\t" 'BEGIN {OFS=","} {foo[$0]++} END {for(x in foo) {print x,foo[x]}}' >> $tpath/foo0.txt
shopt -u extglob
...
grep -r -A 1 --include "*Plugin.xml" "plugin_parent<" * | grep "\- " | sed 's/\(.*\).xml-.*<string>.*\.\(.*\)<\/string>/\1,\2/' > $tpath/foo6.txt
grep -rL --include "*Plugin.xml" "plugin_parent<" * | sed 's/\(.*\).xml/\1,!/' >> $tpath/foo6.txt
