...
shopt -s extglob
cat ../../tdb/!(clockssingest)/*.tdb | ../../scripts/tdb/tdbout -t plugin | sed 's/\./\//g' | awk -F"\t" 'BEGIN {OFS=","} {foo[$0]++} END {for(x in foo) {print x,foo[x]}}' >> $tpath/foo0.txt
shopt -u extglob
...
cat $tpath/foo6.txt | sort -t, > $tpath/blatz6.txt
cat $tpath/foo7.txt | sort -t, > $tpath/blatz7.txt
cat $tpath/foo8.txt | sort -t, > $tpath/blatz8.txt

#echo "Plugin,V,Parent,PV,AUs,Substance,Art.It.,MetadataEx,OpenURL,Status"
#join -e -t, -e EMPTY $tpath/blatz8.txt $tpath/blatz6.txt | join -t, -e EMPTY - $tpath/blatz7.txt | join -t, -e EMPTY - $tpath/blatz0.txt | join -t, -e EMPTY - $tpath/blatz2.txt | join -t, -e EMPTY - $tpath/blatz3.txt | join -t, -e EMPTY - $tpath/blatz4.txt | join -t, -e EMPTY - $tpath/blatz5.txt | join -t, -e EMPTY - $tpath/blatz1.txt | sort -t, -k 5 -nr 
echo "Plugin,Parent,PV,AUs,Substance,Art.It.,MetadataEx,OpenURL,Status"
join -t, $tpath/blatz6.txt $tpath/blatz7.txt | join -t, - $tpath/blatz0.txt | join -t, - $tpath/blatz2.txt | join -t, - $tpath/blatz3.txt | join -t, - $tpath/blatz4.txt | join -t, - $tpath/blatz5.txt | join -t, - $tpath/blatz1.txt | sort -t, -k 4 -nr 
#echo "Plugin,Parent,AUs,Substance,Art.It.,MetadataEx,OpenURL,Status"
#join -t, $tpath/blatz6.txt $tpath/blatz0.txt | join -t, - $tpath/blatz2.txt | join -t, - $tpath/blatz3.txt | join -t, - $tpath/blatz4.txt | join -t, - $tpath/blatz5.txt | join -t, - $tpath/blatz1.txt | sort -t, -k 3 -nr 
